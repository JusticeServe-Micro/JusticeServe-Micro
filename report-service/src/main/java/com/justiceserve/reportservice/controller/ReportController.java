...
public class ReportController {
    ...
    @GetMapping public ResponseEntity<List<Report>> getAll() { return ResponseEntity.ok(repo.findAll()); }
    @GetMapping("/scope/{scope}") public ResponseEntity<List<Report>> getByScope(@PathVariable Report.ReportScope scope) { return ResponseEntity.ok(repo.findByScope(scope)); }
    @GetMapping("/{id}") public ResponseEntity<Report> getById(@PathVariable Long id) { return ResponseEntity.ok(repo.findById(id).orElseThrow()); }
}