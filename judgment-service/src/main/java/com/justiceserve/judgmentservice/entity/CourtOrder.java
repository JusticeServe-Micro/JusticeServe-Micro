...
@Entity @Table(name = "court_orders")
...
public class CourtOrder {
    ...
    @Enumerated(EnumType.STRING) @Column(nullable=false,length=10) private OrderStatus status;
    @PrePersist public void prePersist() { this.date=LocalDate.now(); if (status==null) status=OrderStatus.ACTIVE; }
}