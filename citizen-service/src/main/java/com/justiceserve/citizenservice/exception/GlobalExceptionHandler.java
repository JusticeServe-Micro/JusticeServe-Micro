...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> notFound(ResourceNotFoundException e) {
        return ResponseEntity.status(404).body(err(404, e.getMessage()));
    }

    @ExceptionHandler(BadRequestException.class)
    public ResponseEntity<Map<String, Object>> bad(BadRequestException e) {
        return ResponseEntity.status(400).body(err(400, e.getMessage()));
    }

    @ExceptionHandler(AccessDeniedException.class)
    public ResponseEntity<Map<String, Object>> denied(AccessDeniedException e) {
        return ResponseEntity.status(403).body(err(403, "Access denied"));
    }
    ...
    public ResponseEntity<Map<String, Object>> general(Exception e) {
        return ResponseEntity.status(500).body(err(500, "Error: " + e.getMessage()));
    }

    private Map<String, Object> err(int s, String m) {
        return Map.of("status", s, "message", m, "timestamp", LocalDateTime.now().toString());
    }
}
